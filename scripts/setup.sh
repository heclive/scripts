...
ok=0
	...
	do 	
		echo ""		
		echo "Set the System Time and Date?"
...
software="/media/INTENSO/software"
moderncv="/media/INTENSO/MyFiles/IT/latex/hector/documents/bewerbung/packete/moderncv/"
CV="/media/INTENSO/MyFiles/job/CV/"
#-----------------------------------------MENU------------------------------------------
menu () {
echo ""
echo ""
...
#------------------------------------------ARRAY-----------------------------------------
opt=(	[1]="gnome-terminal -x sh sources.sh"
	[2]="sudo apt-get install gimp -y" 
	[3]="sudo dpkg -i "$software"/libre/DEBs/*.deb" 
	[4]="sudo dpkg -i "$software"/opera/*.deb"
	[5]="sudo apt-get install gconf-editor -y"
	[6]=latex
	[7]=lamp	
...
execute (){
	echo ""
	echo -e $YELLOW "==========EXECUTING ["$i"] ......"$ENDCOLOR 
	echo "" 
	${opt[$i]};
	echo ${opt[$i]}" EXECUTED." >> ~/.$i
	}
lamp () {
localh=$(hostname -I)
...
do 	
		...
		then 
			...
			#--------------copying CV to root--------
			#rm -r /usr/share/texmf-texlive/tex/latex/moderncv/*
			#cp -r ${moderncv}* /usr/share/texmf-texlive/tex/latex/moderncv/
			#texhash
			mkdir /root/CV/
			cp -r ${CV}* /root/CV
			gnome-terminal -x sh -c 'cd /root/CV/;bash'
...
then	 	
	...
	do 	
		echo ""		
		echo "Is the source directory correct? ("/media/INTENSO/")"
		read -s -n 1 -p "Please enter \"y\" or \"n\" to accept." ok
		echo ""
		if [ $ok = "n" ]
		then 
			read -p "Please enter the source directory:" git
		else
			echo ""			
		fi
		#rm ${root}git/env.sh						
		#touch ${root}git/env.sh
		#echo '#!/bin/bash'  >> ${root}git/env.sh
		#echo 'export PATH=$PATH:'${root}'git/'  >> ${root}git/env.sh
		#cd ${root}git	
		#source ./env.sh
	done 	
else		
	...
	echo 'export PATH=$PATH:'${git}'/'  >> ~/env.sh
	echo 'export PATH=$PATH:/media/joomla/texlive/bin/i386-linux'  >> ~/env.sh
	echo 'export MANPATH=$MANPATH:/media/joomla/texlive/texmf-dist/doc/man'  >> ~/env.sh
	echo 'export INFOPATH=$INFOPATH:/media/joomla/texlive/texmf-dist/doc/info'  >> ~/env.sh
	echo 'gnome-terminal -x sh -c '\"'setup.sh; bash'\" >> ~/env.sh #${root}git/'setup.sh'  >> ${root}git/env.sh
	cd ~/	
	source ./env.sh
	exit 0
fi
...
#                                  Pruefe Auswahl und starte Auswahl
if [ -f ~/.$i ]
then 
	read -n 1 -s -p"["$i"] Allready processed, execute again? [y/n]?" ON
	case $ON in 
			y*|J*|Y*|j*) execute;;
			n*|N*) echo "";echo -e $WHITE"Please select number to proceed."$ENDCOLOR;;
			*) read -n 1 -s -p"Wrong Input, please type in [y/n]:" ON;;
	esac 
